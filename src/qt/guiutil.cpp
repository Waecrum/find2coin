...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("findcoin"))
        return false;
...
{
    // Convert findcoin:// to findcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("findcoin://"))
    {
        uri.replace(0, 12, "findcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FindCoin.lnk";
}
...
{
    return GetAutostartDir() / "findcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FindCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  findcoin-qt [" + tr("command-line options") + "]                     " + "\n";
